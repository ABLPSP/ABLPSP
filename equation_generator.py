...
def generator_equations(left_opt_len, right_opt_len, res_opt_len, system_num, label, generate_type):
    ...
    for nums in candidates:
        ...
        #leading zeros
        if (res_opt_len != len(result_num)):# 不能进位
            continue
    ...
    for left_opt_len in range(1, equation_len - (2 + equation_sign_num) + 1):
        for right_opt_len in range(1, equation_len - left_opt_len - (1 + equation_sign_num) + 1):
            ...
            for i in range(repeat_times): #generate more equations
                ...
                print(right_opt_len)
                print(res_opt_len)
                equas=generator_equations(left_opt_len, right_opt_len, res_opt_len, system_num, label, generate_type)
                print(equas)
                ret.extend(equas)
    print(ret)
    return ret
...
None, seed = 0, train_num_per_len = 10, test_num_per_len = 10, is_color = False):
    tmp_file = ""
    tmp_str_file = ""
    if (tmp_file_prev is not None):
        ...
        tmp_str_file = "%s_train_len_%d_test_len_%d_sys_%d_str.pk" % (tmp_file_prev, train_max_equation_len, test_max_equation_len, system_num)
    if (os.path.exists(tmp_file)) and (os.path.exists(tmp_str_file)) and False:
        return pickle.load(open(tmp_file, "rb")),pickle.load(open(tmp_str_file, "rb"))
    ...
    for label in ["positive", "negative"]:
        ...
        ret_str["train:%s" % (label)] = train_equations
        ret_str["test:%s" % (label)] = test_equations
        print("Generated equation image data.")

    if (tmp_file_prev is not None):
        pickle.dump(ret, open(tmp_file, "wb"))
    if (tmp_file_prev is not None):
        pickle.dump(ret_str, open(tmp_str_file, "wb"))
    return ret, ret_str # 返回正确和错误两种情况的图片序列
...
if __name__ == "__main__":
    ...
    data_dirs = ["../dataset/hms_images"] #, "../dataset/cifar10_images"]
    tmp_file_prevs = ["hms_equation_data"] #, "cifar10_equation_data"]
