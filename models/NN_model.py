...
from keras.layers import Dense, Activation, Conv2D, MaxPooling2D, Flatten, BatchNormalization, Dropout, Reshape, UpSampling2D, Input
...
def get_Symbol_net(labels_num, input_shape = (32, 32, 1)):
    h = input_shape[0]
    w = input_shape[1]
    d = input_shape[2]
    model = Sequential()
...
def get_LeNet5_net_refl(labels_num, input_shape=(28, 28, 1)):
    h = input_shape[0]
    w = input_shape[1]
    d = input_shape[2]
    ...
    x=BatchNormalization()(x)# BN is good 
    x=Conv2D(kernel_size=(5, 5), filters=64,  activation='relu', padding='same')(x)
    x=MaxPooling2D(pool_size=(2,2), strides=2, padding='same')(x)
    x=BatchNormalization()(x)# BN is useful
    x=Flatten()(x)
    x=Dense(120, activation='relu')(x)
    x=Dense(84, activation='relu')(x)
    prob=Dense(labels_num, activation='softmax')(x)
    refl=Dense(2, activation='softmax')(x)
    model = Model(inputs=inputs, outputs=[prob, refl])
    return model
